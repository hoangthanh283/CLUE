...
class CORDDataset(Dataset):
    ...
    def _filter_by_labels(self, data: list, label_filter: list[str]) -> list:
        ...
        CORD receipts are densely multi-class (a single receipt carries menu, total,
        sub_total entities at once), so a strict-subset filter keeps zero documents.
        Standard CIL token-classification practice is to retain documents that contain
        at least one target-class entity and relabel every out-of-session entity token
        as background ('O') for this session, so the model only learns the session's
