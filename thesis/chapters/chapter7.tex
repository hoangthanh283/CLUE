...
benchmark against the baselines with a depth-targeting ablation.
\textbf{[TODO: one paragraph stating the effect sizes and whether the targeted
remedy beats the strong replay baseline, once the corrected run is in.]}
Whatever the remedy's margin, the diagnostic contribution stands on its own: it
provides a reusable tool and a shared benchmark for studying forgetting in multimodal
document encoders---together with a strong-baseline context (replay reaching the
oracle) that any targeted method must beat---which the prior, fragmented literature
lacked.
...
the candidate mechanisms reserved in \S\ref{sec:method-candidates}---component-banked
