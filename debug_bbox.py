...
# Add src to path
sys.path.append('./src')
from data.layoutlm_datasets import FUNSDDatasetLoader
...
def main():
    ...
    for i, example in enumerate(train_examples):
        ...
        if example.bboxes:
            ...
            if out_of_range.any():
                print(f"  WARNING: Found out-of-range values!")
                ...
                print(f"  ✓ All bbox values are within [0, 1000] range!")
            ...
            try:
                ...
                print(f"  Final tokenized bbox ranges:")
                ...
                if final_out_of_range.any():
                    print(f"  WARNING: Found out-of-range values in final tokenized bbox!")
                    problem_positions = torch.where(final_out_of_range)
                    for idx in range(min(10, len(problem_positions[0]))):
                        batch, seq, coord = problem_positions[0][idx].item(), problem_positions[1][idx].item(), problem_positions[2][idx].item()
                        print(f"    Position [{batch}, {seq}, {coord}]: {final_bbox[batch, seq, coord].item()}")
                else:
                    print(f"  ✓ All final bbox values are within [0, 1000] range!")
